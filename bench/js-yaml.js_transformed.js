...
(function (global, factory) {
  ...
  function toArray(sequence) {
    if (Array.isArray(sequence)) return sequence;else
  ...
  common.isNothing = isNothing_1;common.isObject = isObject_1;common.toArray = toArray_1;common.repeat = repeat_1;common.isNegativeZero = isNegativeZero_1;common.extend = extend_1;
    ...
    for (index = 0, length = object.length; index < length; index += 1) {
      ...
      for (pairKey in pair) {
        if (_hasOwnProperty$3.call(pair, pairKey)) {
          if (!pairHasKey) pairHasKey = true;else
          return false;
        }
      }
      ...
      if (objectKeys.indexOf(pairKey) === -1) objectKeys.push(pairKey);else
      return false;
    }
  ...
  function generateError(state, message) {
    ...
    };mark.name = state.filename;mark.buffer = state.input.slice(0, -1);mark.position = state.position;mark.line = state.line;mark.column = state.position - state.lineStart;
  ...
  };directiveHandlers.YAML = function handleYamlDirective(state, name, args) {var match, major, minor;if (state.version !== null) {throwError(state, "duplication of %YAML directive");}if (args.length !== 1) {throwError(state, "YAML directive accepts exactly one argument");}match = /^([0-9]+)\.([0-9]+)$/.exec(args[0]);if (match === null) {throwError(state, "ill-formed argument of the YAML directive");}major = parseInt(match[1], 10);minor = parseInt(match[2], 10);if (major !== 1) {throwError(state, "unacceptable YAML version of the document");}state.version = args[0];state.checkLineBreaks = minor < 2;if (minor !== 1 && minor !== 2) {throwWarning(state, "unsupported YAML version of the document");}};directiveHandlers.TAG = function handleTagDirective(state, name, args) {var handle, prefix;if (args.length !== 2) {throwError(state, "TAG directive accepts exactly two arguments");}handle = args[0];prefix = args[1];if (!PATTERN_TAG_HANDLE.test(handle)) {throwError(state, "ill-formed tag handle (first argument) of the TAG directive");}if (_hasOwnProperty$1.call(state.tagMap, handle)) {throwError(state, 'there is a previously declared suffix for "' + handle + '" tag handle');}if (!PATTERN_TAG_URI.test(prefix)) {throwError(state, "ill-formed tag prefix (second argument) of the TAG directive");}try {prefix = decodeURIComponent(prefix);} catch (err) {throwError(state, "tag prefix is malformed: " + prefix);}state.tagMap[handle] = prefix;};
  ...
  /*eslint-disable no-use-before-define*/loader.loadAll = loadAll_1;loader.load = load_1;
  ...
  types.binary = binary;types.float = float;types.map = map;types.null = _null;types.pairs = pairs;types.set = set;types.timestamp = timestamp;types.bool = bool;types.int = int;types.merge = merge;types.omap = omap;types.seq = seq;types.str = str;var safeLoad = renamed("safeLoad", "load");
  ...
  var src = {
    ...
    safeDump: safeDump
  };src.Type = Type;src.Schema = Schema;src.FAILSAFE_SCHEMA = FAILSAFE_SCHEMA;src.JSON_SCHEMA = JSON_SCHEMA;src.CORE_SCHEMA = CORE_SCHEMA;src.DEFAULT_SCHEMA = DEFAULT_SCHEMA;src.load = load;src.loadAll = loadAll;src.dump = dump;src.YAMLException = YAMLException;src.types = types;src.safeLoad = safeLoad;src.safeLoadAll = safeLoadAll;src.safeDump = safeDump;
